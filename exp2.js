...
window.onload = function()
{
    ...
    $(document).keydown(function(e){
        ...
        if(e.which === 68)
        {
            ...
            let pre_graph = JSON.parse(JSON.stringify(graph));

            do_draw_line = false;
            let r_count = DFS(pre_graph, pre_points);
            for(let i=0; i<points.length; i++)
            {
                pre_points = JSON.parse(JSON.stringify(points));
                pre_graph = JSON.parse(JSON.stringify(graph));


                // 删除边
                for(let k=0;k<pre_graph.length; k++)
                {
                    for(let j=pre_graph[k].head.next; j !== null; j = j.next)
                    {
                        if(j.element === i)
                        {
                ...
                // 删除点
                pre_points.splice(i, 1);
                pre_graph.splice(i, 1);


                let now_count = DFS(pre_graph, pre_points);

                if(now_count > r_count)
                {
                    gedian.push(i);
                }
...
function onUp(e)
{
    if(draw_line)
    {
        ...
        drawArrow(ctx, init_x, init_y, x, y, 0, 0, 3, arrow_color);
        let f_point;
...
function draw()
{
    ...
    for(let i=0; i<points.length; i++)
    {
        if(gedian.indexOf(i) === -1)
        {
            drawCircle(ctx, points[i].x, points[i].y, points[i].id, 'red');
        }else
        {
            drawCircle(ctx, points[i].x, points[i].y, points[i].id, 'green');
        }
    ...
    for(let i=0; i<graph.length; i++)
    {
        for(let j=graph[i].head.next; j !== null; j = j.next)
        {
            drawArrow(ctx, points[i].x, points[i].y, points[j.element].x, points[j.element].y,0, 0, 3, arrow_color);
        }
    }
